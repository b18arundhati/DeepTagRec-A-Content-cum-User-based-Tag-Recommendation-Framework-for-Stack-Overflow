...
tag_dict, order, set_tag = utils.create_tag_dict("Body_Title_same.p")
...
tag_output = Dense(units=num_tags, activation="sigmoid", name="tag_output")(Adder)
#tag_output = Dense(units=num_tags, activation="sigmoid", name="tag_output")(Merged_layer)
#tag_output = Dense(units=num_tags, activation="softmax", name="tag_output")(lstm_concat)

model = Model(inputs=[question_input], output=[tag_output])
		...
		for o,y in enumerate(x) :
			...
			if cnt == batch_size:
				...
				#model.save_weights('model4_train_add_Body_Title_weights_gru_epochs10.h5')
                                model.save('model4_train_DeepTagRecContent_usingAdd_sigmoid.h5')
                                model.save_weights('model4_train_DeepTagRecContent_weights_usingAdd_sigmoid.h5')
				x_train = []
				user = []
				y_train = []
...
def test():
	...
	openfile = open("Testing_Body_Title_user.p", "rb")
	x =  pickle.load(openfile)
...
if __name__=="__main__":
	...
	model = load_model('model4_train_DeepTagRecContent_usingAdd_softmax.h5')
	model.load_weights('model4_train_DeepTagRecContent_weights_usingAdd_softmax.h5')#model1_Body_Title_weights_gru_epochs10.h5')
	#repre = user_rep()
	# tin_DeepTagRecContent_usingAdd_sigmoid.h5rain()
